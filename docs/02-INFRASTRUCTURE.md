...
| R-9 | **Cache context auth** | `hola:{env}:auth:user:{user_id}`, TTL 60 detik. Menyimpan status, role, dan `token_version` user setelah JWT tervalidasi. | Cache miss atau Redis mati → context dibaca langsung dari PostgreSQL. Cache tidak pernah menjadi sumber kebenaran auth. |
| R-10 | **Denylist access-token `jti`** | `hola:{env}:auth:denylist:{jti}`, TTL sama dengan sisa umur JWT. Dipakai saat logout sebagai pengurangan jendela revokasi. | Pemeriksaan fail-open; token yang didenylist dapat hidup hingga JWT berakhir (maksimum 15 menit). `token_version` dan refresh-token PostgreSQL tetap mekanisme revokasi utama. |
