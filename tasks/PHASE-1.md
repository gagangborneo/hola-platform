...
- [x] **F0-01** `0,5h` 🔒 — Inisialisasi repo, `pnpm-workspace.yaml`, struktur folder `apps/` + `packages/` + `docs/`, `.gitignore`, `.nvmrc` (Node 20+). *acuan:* [16 § 4.1](../docs/16-CONVENTIONS.md#41-root)
- [x] **F0-02** `0,5h` 🔒 — `tsconfig.base.json` dengan 10 opsi wajib (`strict`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`, `verbatimModuleSyntax`, `noFallthroughCasesInSwitch`, …) + `tsconfig.json` turunan tiap workspace. *acuan:* [16 § 1.1](../docs/16-CONVENTIONS.md#11-konfigurasi)
- [x] **F0-03** `0,5h` 🔒 — `biome.json`: 10 aturan wajib aktif + format (indent 2, width 100, single quote, semicolon `asNeeded`, trailing comma `all`, import organizing). *acuan:* [16 § 2](../docs/16-CONVENTIONS.md#2-biome)
- [x] **F0-04** `0,5h` 🔒 — Override `noRestrictedImports` per app/package (4 baris tabel: web/admin/mobile, shared, api-client, api). *acuan:* [16 § 2 `noRestrictedImports` per app](../docs/16-CONVENTIONS.md#norestrictedimports-per-app)
- [x] **F0-05** `0,5h` 🔒 — `turbo.json` (pipeline `typecheck`/`lint`/`test`/`build`/`dev`) + seluruh 17 perintah root `package.json`. *acuan:* [02 § 11 Perintah pnpm](../docs/02-INFRASTRUCTURE.md#perintah-pnpm-yang-wajib-ada-di-root-packagejson)
- [x] **F0-06** `1h` ⚙️🔒 — `docker-compose.yml`: `postgres:16-alpine`, `redis:7-alpine` dengan **3 flag wajib** (`appendonly yes`, `appendfsync everysec`, `maxmemory-policy noeviction`), `rustfs`, `rustfs-init` (3 bucket), `mailpit`. *acuan:* [02 § 11](../docs/02-INFRASTRUCTURE.md#layanan-di-docker-composeyml-root-repo), [02 § 2](../docs/02-INFRASTRUCTURE.md#2-daftar-container--sumber-daya)
- [x] **F0-07** `0,5h` — `.env.example` lokal yang cocok dengan compose + bagian "cara mulai" di README. *acuan:* [02 § 11 `.env.example` lokal](../docs/02-INFRASTRUCTURE.md#envexample-lokal-nilai-yang-cocok-dengan-compose)

## F0.B — `packages/shared` · 8h

- [x] **F0-08** `1h` 🔒 — `constants/enums.ts`: **seluruh 53 enum** sebagai `as const` object + union type (bukan `enum` TypeScript). *acuan:* [03 § 3](../docs/03-DATA-MODEL.md#3-daftar-enum), [16 BR-TS-05](../docs/16-CONVENTIONS.md#12-aturan-tipe)
- [x] **F0-09** `0,5h` — `constants/error-codes.ts`: katalog `ERROR_CODE` lengkap. *acuan:* [04 § 5 Katalog error code](../docs/04-API-CONTRACT.md#katalog-error-code)
- [x] **F0-10** `0,5h` 🔒 — `constants/queues.ts`: `QUEUE.*` (6 queue) + `JOB.*` (37 job) sebagai union type. *acuan:* [02 § 5.1](../docs/02-INFRASTRUCTURE.md#51-queue), [02 § 5.2](../docs/02-INFRASTRUCTURE.md#52-tabel-job)
- [x] **F0-11** `0,5h` — `constants/limits.ts` + `constants/settings-keys.ts` + `constants/notification-templates.ts`. *acuan:* [16 § 4.3](../docs/16-CONVENTIONS.md#43-packagesshared)
- [x] **F0-12** `0,5h` 🔒 — `redis-keys.ts`: builder **pure** untuk 9 pola key, semuanya berprefiks `hola:{env}:`. Tidak boleh ada string key literal di service. *acuan:* [02 § 4.1](../docs/02-INFRASTRUCTURE.md#41-namespace-key), [16 BR-RD-02](../docs/16-CONVENTIONS.md#7-pola-redis-client)
- [x] **F0-13** `0,5h` — `format/money.ts` (`formatIDR`), `format/date.ts` (`formatWita`, `formatDateRange`), `format/duration.ts` + test
- [x] **F0-14** `0,5h` 🔒 — `utils/round.ts` `roundTo100()` *half-up* + test tabel kasus. *acuan:* [07 § 3.3 P10](../docs/07-MODULE-PAYMENT.md#33-detail-per-step), [08 § 8 Contoh](../docs/08-MODULE-PROMO.md#8-perhitungan-diskon)
- [x] **F0-15** `1h` 🔒 — `utils/slot-grid.ts`: `isSlotAligned()`, `buildSlotGrid()` — **pure**, menerima `now` sebagai parameter (BR-TS-10) + test. *acuan:* [03 § 8.2](../docs/03-DATA-MODEL.md#82-definisi-slot--grid)
- [x] **F0-16** `0,5h` — `utils/iso-week.ts` + test (dipakai `WEEKLY_STREAK` Phase 3, dibuat sekarang agar tidak ada helper waktu kedua)
- [x] **F0-17** `0,5h` — `env/`: schema zod env per app (api, web, admin, mobile) sesuai 4 tabel + script `pnpm check:env`. *acuan:* [02 § 8](../docs/02-INFRASTRUCTURE.md#8-daftar-environment-variable-per-app)
- [x] **F0-18** `0,5h` — `schemas/common.ts`: `paginationQuery` (cursor + offset), `idParam`, `money`, `isoDateTime`, `dateRange`. *acuan:* [04 § 6](../docs/04-API-CONTRACT.md#6-pagination-filter-sorting)
- [x] **F0-19** `0,5h` — `schemas/auth.ts` (register, login, refresh, password, otp) + `types/quote.ts` (bentuk `Quote`). *acuan:* [05](../docs/05-AUTH.md), [07 § 3.1](../docs/07-MODULE-PAYMENT.md#31-kontrak-fungsi)
- [x] **F0-20** `0,5h` 🔴 — Test tabel-driven: setiap konstanta enum **identik** dengan daftar di [03 § 3](../docs/03-DATA-MODEL.md#3-daftar-enum)
- [x] **F0-21** `0,5h` — Verifikasi larangan: `packages/shared` tidak mengimpor `fs`/`path`/`crypto` Node dan tidak menyentuh `process.env` (harus jalan di React Native). *acuan:* [01 § 3.6](../docs/01-ARCHITECTURE.md#36-packagesshared)

## F0.C — `packages/db` · 6h

- [x] **F0-22** `0,5h` — Konfigurasi drizzle-kit, koneksi Drizzle tunggal, mapping kolom uang `bigint → number` (BR-TS-07), helper UUID v7. *acuan:* [16 § 4.2](../docs/16-CONVENTIONS.md#42-appsapi), [03 § 2](../docs/03-DATA-MODEL.md#2-konvensi-tipe-data--kolom-standar)
- [x] **F0-23** `0,5h` 🔒 — Migration 0001: **53 tipe enum PostgreSQL**, nilainya identik dengan F0-08. *acuan:* [03 § 3](../docs/03-DATA-MODEL.md#3-daftar-enum)
- [x] **F0-24** `1h` 🔒 — Tabel identity: `users` (+ `ck_users_identifier`, UNIQUE email lowercase & phone), `refresh_tokens` (C-23 + index), `customer_profiles`. *acuan:* [03 § 5](../docs/03-DATA-MODEL.md#5-entitas-identity--access)
- [x] **F0-25** `1h` 🔒 — Tabel venue & lapangan: `venues`, `sports`, `courts` (+ C-24), `court_operating_hours` (UNIQUE `(court_id, day_of_week)`), `special_dates`, `addons`. *acuan:* [03 § 6](../docs/03-DATA-MODEL.md#6-entitas-venue-court-pricing)
- [x] **F0-26** `0,5h` — Tabel `price_rules` + 3 CHECK (`ck_price_rules_scope`, `_specific_date`, `_time_range`). *acuan:* [03 § 6 `price_rules`](../docs/03-DATA-MODEL.md#price_rules)
- [x] **F0-27** `0,5h` — Tabel sistem: `app_settings`, `audit_logs`, `idempotency_records` (C-22), `otp_challenges`, `password_reset_tokens`. *acuan:* [03 § 17](../docs/03-DATA-MODEL.md#17-entitas-notification-media-system)
- [x] **F0-28** `0,5h` — Tabel media & notifikasi: `media_files`, `notification_templates`, `notifications` (C-19), `push_tokens`. *acuan:* [03 § 17](../docs/03-DATA-MODEL.md#17-entitas-notification-media-system)
- [x] **F0-29** `0,5h` — Sequence kode manusia: `seq_booking_code`, `seq_payment_code`, `seq_refund_code`, `seq_invoice_number`, `seq_journal_entry`, `seq_employee_number`. *acuan:* [03 § 2 Format kode manusia](../docs/03-DATA-MODEL.md#format-kode-manusia)
- [x] **F0-30** `0,5h` — Index fondasi yang relevan Phase 0 (`idx_audit_logs_entity`, `idx_notifications_status_created`, index `refresh_tokens`). *acuan:* [03 § 18](../docs/03-DATA-MODEL.md#index-untuk-performa-minimal)
- [x] **F0-31** `0,5h` 🔴 — Test integrasi: enum TypeScript ≡ `pg_enum` di database uji (menangkap drift F0-08 vs F0-23)

## F0.D — `apps/api` core · 7h

- [x] **F0-32** `0,5h` 🔒 — Struktur `apps/api`, `env.ts` (validasi zod, **gagal keras saat boot**), `index.ts`, `app.ts` + ekspor `AppType`. *acuan:* [16 § 4.2](../docs/16-CONVENTIONS.md#42-appsapi), [02 § 8](../docs/02-INFRASTRUCTURE.md#8-daftar-environment-variable-per-app)
- [x] **F0-33** `0,5h` 🔒 — `config/db.ts`, `config/redis.ts` (**dua koneksi terpisah**: umum + BullMQ), helper `safeRedis(op, fallback)` dengan log `warn` + metrik `redis_degraded_total`. *acuan:* [16 BR-RD-01/BR-RD-03](../docs/16-CONVENTIONS.md#7-pola-redis-client)
- [x] **F0-34** `0,5h` — `config/logger.ts` pino: field wajib per request & per job, **redaction 11 kunci** + `config/sentry.ts` (error yang *diharapkan* tidak dikirim). *acuan:* [02 § 9](../docs/02-INFRASTRUCTURE.md#9-observability)
- [x] **F0-35** `1h` 🔒 — `lib/errors.ts`: kelas `AppError` + factory per `ERROR_CODE` + `UniqueViolationError` (membawa nama constraint) + `middleware/error-handler.ts` sebagai **satu-satunya** tempat error → response. *acuan:* [04 § 5](../docs/04-API-CONTRACT.md#5-format-error), [16 BR-SV-22](../docs/16-CONVENTIONS.md#53-bentuk-repository)
- [x] **F0-36** `0,5h` — `lib/response.ts` (envelope `{data}`/`{error}` + header standar) + `lib/pagination.ts` (cursor encode/decode, offset). *acuan:* [04 § 4](../docs/04-API-CONTRACT.md#4-format-response-sukses), [04 § 6](../docs/04-API-CONTRACT.md#6-pagination-filter-sorting)
- [x] **F0-37** `1h` 🔒 — `lib/transaction.ts` (`withTransaction` + callback `afterCommit` untuk enqueue) + `lib/codes.ts` (generator kode dari sequence, **di dalam** transaksi) + `lib/time.ts` (WITA, tanggal bisnis, grid). *acuan:* [16 BR-SV-13/BR-SV-14](../docs/16-CONVENTIONS.md#52-bentuk-service)
- [x] **F0-38** `0,5h` — `middleware/request-id.ts` (ULID + header `X-Request-Id`) + `middleware/logger.ts`
- [x] **F0-39** `1h` — `middleware/rate-limit.ts`: fixed window via Lua `EVALSHA`, **12 bucket**, identifier user/ip, **fail-open** + metrik + header `Retry-After`. *acuan:* [02 § 4.5](../docs/02-INFRASTRUCTURE.md#45-konfigurasi-rate-limit), [16 BR-RD-08](../docs/16-CONVENTIONS.md#7-pola-redis-client)
- [x] **F0-40** `0,5h` — `middleware/idempotency.ts`: Redis fast path 24 jam + `idempotency_records` (C-22) sebagai jaminan durabel + header `X-Idempotent-Replay`. *acuan:* [04 § 8](../docs/04-API-CONTRACT.md#8-idempotency--concurrency)
- [x] **F0-41** `0,5h` — `GET /healthz`, `GET /readyz` (cek PG + Redis, laporkan `degraded`), `GET /healthz/worker`, `GET /internal/metrics` (header `X-Internal-Token`). *acuan:* [02 § 2](../docs/02-INFRASTRUCTURE.md#2-daftar-container--sumber-daya), [02 § 9](../docs/02-INFRASTRUCTURE.md#9-observability)
- [x] **F0-42** `0,5h` — `GET /config/public`: `midtrans_client_key`, `server_time`, `min_supported_mobile_version`, timezone, teks kebijakan, flag fitur. *acuan:* [04 § 9.2](../docs/04-API-CONTRACT.md#92-konfigurasi-publik)

## F0.E — Auth & RBAC · 11h

- [x] **F0-43** `0,5h` — Hashing argon2id (`memoryCost=19456`, `timeCost=2`, `parallelism=1`) + `PASSWORD_PEPPER` + validasi password (≥ 8, bukan 10.000 terlemah, bukan email/nama). *acuan:* [05 § 8 Password](../docs/05-AUTH.md#password)
- [x] **F0-44** `0,5h` — `POST /auth/register` (hanya membuat role `customer`, respons **anti-enumerasi** untuk email yang sudah ada). *acuan:* [05 § 8](../docs/05-AUTH.md#8-registrasi--login)
- [x] **F0-45** `1h` — `POST /auth/login` + `users.failed_login_count`, `locked_until` (10 gagal → kunci 15 menit), progressive delay 1/2/4/8 s, notifikasi email saat lockout. *acuan:* [05 § 8 Brute force](../docs/05-AUTH.md#brute-force--lockout)
- [x] **F0-46** `1h` 🔒 — Penerbitan refresh token: 32 byte acak → base64url, simpan `token_hash` SHA-256, `family_id`, cookie `HttpOnly; Secure; SameSite=Lax; Path=/api/v1/auth` (web) atau body (mobile). *acuan:* [05 § 4 Refresh token](../docs/05-AUTH.md#refresh-token-opaque)
- [x] **F0-47** `1h` 🔒 — `POST /auth/refresh`: rotasi wajib (T-3) + **deteksi reuse** (T-4: cabut seluruh `family_id` + `token_version + 1`) + batas 10 sesi aktif (T-5). *acuan:* [05 § 5](../docs/05-AUTH.md#5-siklus-hidup-token)
...
- [x] **F0-50** `1h` 🔒 — `middleware/authenticate.ts` sesuai flowchart: verifikasi JWT → denylist `jti` (fail-open) → user context (cache Redis 60 s, miss → PostgreSQL) → cek `users.status` → cek `token_version`. *acuan:* [05 § 4 Verifikasi access token](../docs/05-AUTH.md#verifikasi-access-token-middleware-authenticate)
- [x] **F0-51** `0,5h` 🔒 — `middleware/require-role.ts` + allowlist route publik + **pemeriksaan saat boot** yang membuat aplikasi gagal start bila ada route tanpa guard dan tanpa allowlist. *acuan:* [16 BR-SV-03](../docs/16-CONVENTIONS.md#51-bentuk-route)
- [x] **F0-52** `0,5h` — Helper otorisasi kepemilikan: pola `findByIdForUser(tx, id, userId)` vs `findByIdAdmin(tx, id)`, filter **di WHERE** bukan di JS, `404` untuk customer / `403` untuk staff. *acuan:* [05 § 7](../docs/05-AUTH.md#7-otorisasi-berbasis-kepemilikan)
- [x] **F0-53** `0,5h` — Pola serializer per role (`viewer: { role, userId }`) + mekanisme field-level filtering (`◐`) yang akan dipakai seluruh modul. *acuan:* [05 § 7 Field-level filtering](../docs/05-AUTH.md#field-level-filtering-)
- [x] **F0-54** `0,5h` ⛔ D-04 — Endpoint OTP (`/auth/otp/request`, `/auth/otp/verify`) bertipe lengkap tetapi mengembalikan `403 FEATURE_DISABLED`, + tabel `otp_challenges`. *acuan:* [05 § 8 OTP](../docs/05-AUTH.md#otp)
- [x] **F0-55** `1h` — Modul `admin/users`: list, create (staff/admin/tenant), patch role/status (**wajib** menaikkan `token_version`, A-2), `POST /admin/users/{id}/revoke-sessions` + penulis `audit_logs` yang dipakai seluruh modul. *acuan:* [05 § 6.14](../docs/05-AUTH.md#614-sistem--media), [05 § 9 S-11](../docs/05-AUTH.md#9-keamanan-tambahan)
- [ ] **F0-56** `1,5h` 🔴 — Test auth: T-1…T-12, A-1…A-15, dan **RBAC tabel-driven** untuk seluruh endpoint Phase 0. *acuan:* [05 § 5](../docs/05-AUTH.md#5-siklus-hidup-token), [05 § 10](../docs/05-AUTH.md#10-edge-cases), [16 BR-TT-12](../docs/16-CONVENTIONS.md#82-aturan-test)

## F0.F — BullMQ, notifikasi, mail · 4,5h

- [x] **F0-57** `1h` ⚙️🔒 — `config/queues.ts` (6 producer, dibagikan lewat `ctx.queues`), `worker.ts` (registrasi seluruh `Worker` + `upsertJobScheduler` dengan `jobId` tetap + `tz: 'Asia/Makassar'`), graceful shutdown `SIGTERM` maks 30 detik. *acuan:* [16 § 6.1](../docs/16-CONVENTIONS.md#61-struktur), [02 § 5.3](../docs/02-INFRASTRUCTURE.md#53-ketahanan-job-terhadap-kehilangan-redis)
- [x] **F0-58** `0,5h` 🔴 — Test: konstanta `JOB.*` ≡ job yang benar-benar terdaftar di `jobs/index.ts` (BR-BQ-03)
- [x] **F0-59** `0,5h` — Provider mail: port `MailAdapter` + implementasi `resend` / `smtp` / `console`. *acuan:* [02 § 7](../docs/02-INFRASTRUCTURE.md#7-notifikasi), [16 § 4.2 `providers/`](../docs/16-CONVENTIONS.md#42-appsapi)
- [x] **F0-60** `1h` — Modul `notifications`: `notification_templates`, penulisan baris `inapp` sinkron untuk setiap kanal, **dedupe wajib** C-19, endpoint inbox (`GET /me/notifications`, `/read`, `/read-all`), quiet hours 22:00–07:00 untuk non-transaksional. *acuan:* [02 § 7 Aturan notifikasi](../docs/02-INFRASTRUCTURE.md#aturan-notifikasi)
- [x] **F0-61** `0,5h` — J-25 `notification.sendEmail` + J-36 `notification.retryStuckNotifications` (sweeper resmi). *acuan:* [02 § 5.2](../docs/02-INFRASTRUCTURE.md#52-tabel-job), [02 § 5.3](../docs/02-INFRASTRUCTURE.md#53-ketahanan-job-terhadap-kehilangan-redis)
- [x] **F0-62** `0,5h` — J-31 `system.cleanupExpiredTokens` (cron 04:00 WITA)
- [x] **F0-63** `0,5h` — `bullboard.ts` + basic auth + IP allowlist (ia bisa melihat payload job, S-8). *acuan:* [05 § 9 S-8](../docs/05-AUTH.md#9-keamanan-tambahan)

## F0.G — `packages/api-client` · 1,5h

- [x] **F0-64** `0,5h` — `createHolaClient({ baseUrl, getAccessToken, onUnauthorized })` + `import type { AppType }` (type-only, dijaga `verbatimModuleSyntax`) + pemetaan response error → `HolaApiError`. *acuan:* [01 § 3.7](../docs/01-ARCHITECTURE.md#37-packagesapi-client)
...
- [x] **F0-66** `0,5h` ⚙️ — `config/storage.ts` (klien S3, `S3_FORCE_PATH_STYLE` untuk RustFS) + pemeriksaan keberadaan 3 bucket saat boot. *acuan:* [02 § 6](../docs/02-INFRASTRUCTURE.md#6-object-storage)
- [x] **F0-67** `1h` — `POST /media/presign` + `POST /media/{id}/confirm` (HEAD object, verifikasi mime & ukuran) + `GET`/`DELETE /media/{id}`, allowlist mime & batas ukuran per `kind`, RBAC per `kind`. *acuan:* [02 § 6 Alur upload](../docs/02-INFRASTRUCTURE.md#alur-upload-presigned), [05 § 6.14](../docs/05-AUTH.md#614-sistem--media)
- [x] **F0-68** `0,5h` — J-32 `system.cleanupOrphanUploads` (`pending` > 24 jam)

## F0.I — `apps/web` shell · 2,5h

- [x] **F0-69** `0,5h` — Scaffold Next.js App Router, layout, styling dasar, security headers S-4 (CSP, nosniff, Referrer-Policy, X-Frame-Options). *acuan:* [16 § 4.4](../docs/16-CONVENTIONS.md#44-appsweb--appsadmin), [05 § 9](../docs/05-AUTH.md#9-keamanan-tambahan)
...
- [x] **F0-73** `0,5h` — Scaffold + layout + navigasi yang berubah per role (`admin` / `staff` / `tenant`). *acuan:* [01 § 3.3](../docs/01-ARCHITECTURE.md#33-appsadmin--nextjs-app-router-back-office)
- [x] **F0-74** `0,5h` — Halaman login + guard rute per role (UX saja; keamanan tetap di API — [05 § 1](../docs/05-AUTH.md#1-prinsip))
- [x] **F0-75** `1h` 🔒 — Komponen tabel data generik yang dipakai seluruh modul admin: offset pagination, filter, sort, pencarian `q`, keadaan kosong/error. *acuan:* [04 § 6.2](../docs/04-API-CONTRACT.md#62-offset-untuk-tabel-admin)
...
- [x] **F0-78** `1h` ⚙️🔒 — `ci.yml`: `pnpm install --frozen-lockfile` → `typecheck` → `lint` (`biome ci`) → `test` → `build`, dengan **service container** postgres + redis untuk test integrasi. *acuan:* [02 § 3 GitHub Actions](../docs/02-INFRASTRUCTURE.md#github-actions--workflow-yang-ada)
- [x] **F0-79** `0,5h` 🔴 — Job `guard-db-boundary` (`grep -R` pola terlarang di `apps/*/src`) **dan buktikan gagal** dengan commit percobaan di branch throwaway (DoD-0-03). *acuan:* [01 § 2](../docs/01-ARCHITECTURE.md#2-aturan-keras-semua-akses-database-lewat-appsapi)
- [x] **F0-80** `0,5h` 🔴 — Job `check:env` **dan buktikan gagal** saat satu kunci dihapus dari `.env.example` (DoD-0-04)

## F0.L — Deploy · 4,5h

> **[DITUNDA — menunggu instruksi deploy/publish]** Artefak yang dapat dibangun dan diuji lokal
> tetap dikerjakan. Provisioning VPS/Dokploy, DNS/domain, deploy, dan verifikasi production
> di bawah ini sengaja tetap belum dicentang sampai user memberi instruksi untuk publish.

- [ ] **F0-81** `1h` ⚙️ ⛔ K-02 — Provisioning VPS: Docker, Dokploy, firewall, swap, monitor disk (alert di 80% — E-6)
- [x] **F0-82** `1h` ⚙️ — `Dockerfile` untuk `apps/api` (multi-stage; container worker memakai image yang sama dengan `CMD ["node","dist/worker.js"]`), `apps/web`, `apps/admin`. *acuan:* [02 § 2](../docs/02-INFRASTRUCTURE.md#2-daftar-container--sumber-daya)
- [ ] **F0-83** `1h` ⚙️ ⛔ K-01 — Traefik: routing 6 host, TLS Let's Encrypt, HSTS `max-age=31536000; includeSubDomains`. *acuan:* [02 § 1 Domain & routing](../docs/02-INFRASTRUCTURE.md#domain--routing)
- [ ] **F0-84** `0,5h` ⚙️ — Env & secret di Dokploy per container + **release step** `pnpm db:migrate` (sekali per deploy, **bukan** di `CMD`). *acuan:* [02 § 3 Aturan deployment](../docs/02-INFRASTRUCTURE.md#aturan-deployment)
...
> **[DITUNDA — menunggu instruksi deploy/publish]** Kontrak, job, workflow, dan runbook lokal
> dapat diselesaikan; lifecycle R2 dan drill dengan dump produksi hanya dilakukan saat layanan
> produksi sudah diminta untuk disiapkan.

- [ ] **F0-87** `1h` ⚙️ — Container `hola-backup` (`postgresql-client` + `rclone`), kredensial R2, bucket `hola-backup` + lifecycle rule (30 harian + 12 bulanan). *acuan:* [02 § 10 Strategi backup](../docs/02-INFRASTRUCTURE.md#strategi-backup)
...
- [x] **F0-89** `0,5h` — Runbook `docs/runbooks/restore.md`: 5 langkah konkret + **post-restore checklist Z1–Z7**. *acuan:* [02 § 10 Prosedur restore](../docs/02-INFRASTRUCTURE.md#prosedur-restore-wajib-terdokumentasi--pernah-diuji)
...
> **[DITUNDA — menunggu instruksi deploy/publish]** Integrasi kode sudah dapat diverifikasi
> lokal. Konfigurasi Uptime Kuma/healthchecks eksternal dan penerimaan event Sentry produksi
> menunggu instruksi deploy/publish.

- [ ] **F0-92** `0,5h` ⚙️ ⛔ D-08, K-13 — Uptime Kuma + **8 monitor wajib** + kanal alert yang benar-benar dibaca. *acuan:* [02 § 9 Monitor yang wajib ada](../docs/02-INFRASTRUCTURE.md#uptime-monitoring--butuh-keputusan-client-d-08)
...
- [x] **F0-95** `1h` — Seed bagian 1 (**deterministik**, ID tetap, tanpa random): venue, 3 sports, 7 courts, `court_operating_hours` 06:00–23:00, `price_rules` contoh, `addons`. *acuan:* [02 § 11 Isi seed data dev](../docs/02-INFRASTRUCTURE.md#isi-seed-data-dev-pnpm-dbseed)
...
- [ ] **F0-98** `1h` 🔴 **[DITUNDA — deploy/publish]** — Verifikasi **12 butir DoD Phase 0** + kumpulkan bukti (log, tangkapan layar, isi `restore-drill-log.md`). *acuan:* [ROADMAP § 3.1](ROADMAP.md#31-definition-of-done-phase-0-sebelum-publish-terukur-semuanya-wajib)
- [x] **F0-99** `0,5h` — `docs/runbooks/deploy.md` + bagian pengembangan di README

> **Gerbang Phase 0 → Phase 1 (mode local-first):** P1-01 boleh dimulai setelah fondasi lokal
> terverifikasi — terutama setup mesin bersih, CI, batas database, validasi env, auth/RBAC,
> serta artefak backup/restore dan observability yang dapat diuji lokal — dan pertanyaan Phase 1
> telah dikirim. Item eksternal produksi **F0-81, F0-83–84, F0-86–87, F0-90, F0-92–94, dan F0-98**
> sengaja tidak memblokir pengembangan lokal selama berstatus ditunda.
>
> **Gerbang deploy/publish:** sebelum staging/produksi, akses publik, atau pembayaran nyata,
> seluruh 12 DoD Phase 0 tetap wajib tercentang dengan bukti. Backup yang belum diuji, CI yang
> belum menegakkan batas arsitektur, atau observability produksi yang belum diverifikasi tidak
> boleh dilewati pada tahap tersebut.
...
- [x] **P1-47** `0,5h` 🔒 — Port `PaymentProvider` (5 operasi) + `ManualProvider` (tidak memanggil API mana pun). *acuan:* [07 § 2 Kontrak port](../docs/07-MODULE-PAYMENT.md#kontrak-port-paymentprovider)
- [x] **P1-48** `1h` 🔒 ⛔ K-04, K-05 — `MidtransProvider`: `createTransaction` (Snap), `getTransactionStatus`, `parseWebhook` (SHA-512 **timing-safe**, `gross_amount` dipakai apa adanya), `capabilities` dari `app_settings.refund_api_supported_methods`. *acuan:* [07 § 5.3](../docs/07-MODULE-PAYMENT.md#53-verifikasi-signature), [07 § 7.2](../docs/07-MODULE-PAYMENT.md#72-dukungan-refund-per-metode-pembayaran)
- [x] **P1-49** `1h` 🔒 — `POST /payments`: BR-P-10…BR-P-19. Urutan **wajib**: INSERT `payments` **dulu**, baru panggil gateway (E-2). `item_details` dibangun dari `quote_snapshot.lines` termasuk baris diskon **negatif** agar totalnya sama dengan `gross_amount` (BR-P-16, BR-PR-55)
- [x] **P1-50** `0,5h` — Total Rp 0 → **tidak** memanggil gateway; payment `provider='manual'`, `method='cash'`, `amount=0`, `status='paid'`, payable langsung dikonfirmasi (BR-P-18)
- [x] **P1-51** `0,5h` 🔒 — `applyPaymentTransition()` **pure** (menolak transisi tidak sah, no-op untuk transisi ke status yang sama) + pemetaan 11 baris status Midtrans + `needs_manual_review` untuk `capture`+`challenge` + test. *acuan:* [07 § 4.3](../docs/07-MODULE-PAYMENT.md#43-state-machine-payment), [07 § 4.4](../docs/07-MODULE-PAYMENT.md#44-pemetaan-status-midtrans--payment_status)
- [x] **P1-52** `1,5h` 🔒 — `payments.markPaid()` dalam **satu transaksi**: `payments → paid` (dengan `WHERE status='pending'`, BR-P-35), payable → `confirmed`, `slot_claims held → confirmed` (+ `hold_expires_at = NULL`), `promo_redemptions → applied`, `INSERT finance_events`. Enqueue notifikasi & job **setelah** commit. *acuan:* [07 BR-P-34](../docs/07-MODULE-PAYMENT.md#52-aturan-idempotency)
- [x] **P1-53** `1h` 🔒 — Handler HTTP `POST /webhooks/midtrans`: verifikasi signature → bentuk `provider_event_id` → `INSERT payment_webhook_events ON CONFLICT DO NOTHING` → enqueue J-05 (`jobId = wh-midtrans-{sha256(eventId)}`; tanpa `:` sesuai batasan BullMQ v5) → `200`. **< 1 detik, tanpa logika bisnis, tanpa panggilan jaringan keluar** (BR-P-38). *acuan:* [07 § 5.1](../docs/07-MODULE-PAYMENT.md#51-pembagian-tugas-yang-wajib), [04 § 10](../docs/04-API-CONTRACT.md#10-konvensi-webhook)
- [x] **P1-54** `1h` 🔒 — J-05 `payment.processWebhook`: `SELECT … FOR UPDATE` → keluar bila `processed_at` terisi → cari payment → verifikasi `gross_amount` → transisi → `markPaid()`; tangani `unknown_order` (BR-P-36) dan `amount_mismatch` (BR-P-37) dengan alert, **tanpa** mengonfirmasi
- [x] **P1-55** `0,5h` — `POST /payments/manual` (BR-P-20…BR-P-26, `Idempotency-Key` wajib, memakai fungsi `markPaid()` yang **sama**) + `POST /payments/{id}/cancel` + `/sync`
- [x] **P1-56** `0,5h` ⚙️ — `POST /dev/simulate-webhook`: bentuk payload valid + `signature_key` benar, panggil handler yang sama; guard yang **gagal saat boot** bila `APP_ENV !== 'local'` (S-12). *acuan:* [02 § 11 Cara menguji webhook](../docs/02-INFRASTRUCTURE.md#cara-menguji-webhook-midtrans-di-lokal)
...
- [ ] Sudah membaca [00-OVERVIEW.md](../docs/00-OVERVIEW.md), [01-ARCHITECTURE.md](../docs/01-ARCHITECTURE.md), [03-DATA-MODEL.md](../docs/03-DATA-MODEL.md), [16-CONVENTIONS.md](../docs/16-CONVENTIONS.md)
- [ ] Jawaban client atas **K-01** (domain) dan **K-02** (akun infrastruktur) sudah diterima — wajib sebelum F0-81/deploy-publish, tetapi tidak memblokir pengembangan lokal

### Sebelum P1-01 (gerbang pengembangan lokal Phase 0 → 1)

- [ ] Fondasi lokal yang relevan telah terverifikasi: setup mesin bersih, CI, batas database, validasi env, dan auth/RBAC
- [ ] Artefak backup/restore dan observability lokal tersedia; pekerjaan eksternal produksi tercatat **ditunda menunggu instruksi deploy/publish**
- [ ] Pertanyaan client untuk Phase 1 sudah dikirim: **D-01, D-06, D-07, K-04, K-06, K-07, K-08, K-09, K-10, K-11, K-13**

### Sebelum deploy/publish (gerbang produksi Phase 0)
