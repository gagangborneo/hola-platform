...
- [ ] **P1-47** `0,5h` 🔒 — Port `PaymentProvider` (5 operasi) + `ManualProvider` (tidak memanggil API mana pun). *acuan:* [07 § 2 Kontrak port](../docs/07-MODULE-PAYMENT.md#kontrak-port-paymentprovider)
...
- [ ] **P1-51** `0,5h` 🔒 — `applyPaymentTransition()` **pure** (menolak transisi tidak sah, no-op untuk transisi ke status yang sama) + pemetaan 11 baris status Midtrans + `needs_manual_review` untuk `capture`+`challenge` + test. *acuan:* [07 § 4.3](../docs/07-MODULE-PAYMENT.md#43-state-machine-payment), [07 § 4.4](../docs/07-MODULE-PAYMENT.md#44-pemetaan-status-midtrans--payment_status)
...
- [ ] **P1-56** `0,5h` ⚙️ — `POST /dev/simulate-webhook`: bentuk payload valid + `signature_key` benar, panggil handler yang sama; guard yang **gagal saat boot** bila `APP_ENV !== 'local'` (S-12). *acuan:* [02 § 11 Cara menguji webhook](../docs/02-INFRASTRUCTURE.md#cara-menguji-webhook-midtrans-di-lokal)
